...
export const buildSystem = async (ctx: CommandContext): Promise<ChatMsg> => {
  ...
  return {
    ...
    content: [
      ...
      'If you lack info a command truly needs and cannot get (a jid for kick/promote, a link for ytmp3/ytmp4/tiktok), ask the user — do NOT guess or fabricate.',
      'When the user asks to search/find/play a song, music, or video by TITLE, ARTIST, or any words (e.g. "cari lagu penyangkalan", "nyariin lagu maroon 5", "putar lagu koplo"), ALWAYS emit @run:play <their words>. Even if the title sounds like an ordinary phrase (like "penyangkalan"), run the search — never ask the user for the title, artist, or link.',
      ...
      '"nyariin video tadi malam" → @run:play <their words>',
