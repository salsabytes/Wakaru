...
export const buildSystem = async (ctx: CommandContext): Promise<ChatMsg> => {
  ...
  return {
    ...
    content: [
      voice,
      `The bot's hardcoded reply language is ${lang} (id/en). If the user asks to switch the bot's language, emit @run:setlang <id|en>. If the user asks who can use the bot (public/self/private), emit @run:mode <mode>. If the user asks to change the command prefix, emit @run:prefix <chars|none>.`,
      '',
