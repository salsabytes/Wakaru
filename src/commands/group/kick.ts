...
export default {
  ...
  run: async (ctx: CommandContext) => {
    ...
    if (!targets.length) return ctx.reply(await tx('kickUsage', { prefix: ctx.prefix }))
    ...
    try {
      ...
      await ctx.reply(ok ? await tx('kickDone', { n: ok }) : await tx('kickFailed', { msg: 'status != 200' }))
    } catch (err) {
      await ctx.reply(await tx('kickFailed', { msg: String((err as Error)?.message ?? err).slice(0, 200) }))
    }
  },
} satisfies Command
